...
export class GamePlayComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  ngOnInit() {
    ...
    this.roomCharactersSubscription = this.roomsService.getRoomCharacters().subscribe(characters => {
    ...
    // real-time services uses this to send messages to other players
    ...
    this.characterService.getLocation().pipe(
      filter((newLocation) => newLocation !== null && newLocation !== this.currentLocation),
      distinctUntilChanged(),
      switchMap((newLocation) => {
        console.log("Switching location to:", newLocation);
        this.currentLocation = newLocation;
        return this.updateLocationAndListenToEvents(newLocation as string); // Cast to string to satisfy TypeScript
      })
    ).subscribe();
  ...
  ngOnDestroy() {
    ...
    // Unsubscribe from the roomCharactersSubscription
    if (this.roomCharactersSubscription) {
      this.roomCharactersSubscription.unsubscribe();
    }
  }

  private updateLocationAndListenToEvents(newLocation: string): Observable<void> {
    console.log("Updating location and listeners for:", newLocation);
    if (!newLocation) {
      console.error('New location is null.');
    ...
    console.log("Calling RoomsService.updateLocation with:", newLocation);
    return from(this.roomsService.updateLocation(newLocation));
  }
  ...
  get characterListAsString(): string {
    return this.roomCharacters.length > 0 ? 'Also here:  ' + this.roomCharacters.map(character => `<span class="character-name">${character.characterName}</span>`).join(' and ') : '';
  } */
